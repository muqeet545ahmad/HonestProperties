...
const userController = {
  ...
  readProfile: (req, res) => {
    ...
    userController.findById(userId, user => {
      if (user !== null) {
        ...
        userData.profile_image_path = user.profile_image_path;
        userData.primary_property_id = user.primary_property_id;
  ...
  hardDelete: async (req, res) => {
    ...
    const propertyList = await models.Property.findAll({ where: { user_id: userId } });

    propertyList.forEach(property => {
      var property_id = property.id;

      // Delete DB: PropertyInfos (by property_id)
      models.PropertyInfo.destroy({ where: { property_id: property_id } });

      // Delete DB: PropertyProfiles (by property_id)
      models.PropertyProfile.destroy({ where: { property_id: property_id } });

      // Delete DB: Rooms (by property_id)
      models.Room.destroy({ where: { property_id: property_id } });
    });

    // Delete DB: Properties (by user_id)
    models.Property.destroy({ where: { user_id: userId } });

    // Delete DB: FileStats (by user_id)
    models.FileStats.destroy({ where: { user_id: userId } });

    // Delete DB: UserVerifications (by user_id)
    models.UserVerification.destroy({ where: { user_id: userId } });

    // Delete DB: Users (by id)
    models.User.destroy({ where: { id: userId } });

    // Remove directory: User Directory
    const dir1 = process.env.USER_DIR_PATH + userId;
    userController.removeDirectory(dir1, is_success => {
      if (is_success) {
        console.log('User directory removed successfully.');
      } else {
        console.log('User directory removal failed.');
      }
    });

    // Remove directory: User Profile Directory
    var dir2 = process.env.USER_PROFILE_DIR_PATH + userId;
    userController.removeDirectory(dir2, is_success => {
      if (is_success) {
        console.log('User Profile directory removed successfully.');
      } else {
        console.log('User Profile directory removal failed.');
      }
    });
  ...
  removeDirectory: (dir, res) => {
    // delete directory recursively
    fs.rm(dir, { recursive: true }, err => {
      if (err) {
        console.log(`Failed to delete ${dir}, error: ${err}.`)
        res(false);
      } else {
